...
def download(port, cert_var, cert_length_var, output, use_store, keep_dupes, domain):
    ...
    output.write(cert_util.x509_to_header(down_certs, cert_var, cert_length_var, keep_dupes, os.path.splitext(output.name)[0].upper(), domains=domain))
...
def convert(cert_var, cert_length_var, output, use_store, keep_dupes, cert):
    ...
    # Combine PEMs and write output header.
    output.write(cert_util.x509_to_header(root_certs, cert_var, cert_length_var, keep_dupes, os.path.splitext(output.name)[0].upper()))
