...
def get_label(cert):
    """ Get the label of a cert from it's subject components,
    basically trying a few in order until we get at least one
    string.
    """
    com = dict(cert.get_subject().get_components())
    if b'CN' in com:
        return com[b'CN'].decode("utf-8")
    elif b'OU' in com:
        return com[b'OU'].decode("utf-8")
    elif b'O' in com:
        return com[b'O'].decode("utf-8")
...
def x509_to_header(x509Certs, cert_var, cert_length_var, keep_dupes, guard_name, domains=None):
    ...
    controls the name of the cert data length variable/define, guard_name is the name to use
